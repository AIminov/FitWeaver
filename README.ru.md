...
| Вкладка | Возможности |
|---------|-------------|
| **📅 Календарь** | Месячный вид тренировок из YAML-плана; клик на тренировку — подробности; все CLI-команды доступны из боковой панели |
| **🤖 LLM Генератор** | Подключить локальную LLM (LM Studio / Ollama), вставить текст плана, сгенерировать и просмотреть YAML, а затем собрать FIT или загрузить в Garmin одной кнопкой |
| **🧱 Конструктор** | Собрать новую тренировку из блоков и шаблонов; `back_to_offset` для повторов вычисляется автоматически |
| **🏃 Garmin Connect** | Живой список тренировок через API; мультивыделение и массовое удаление; прокрутка колесом мыши |

Боковая панель со скроллом содержит настройки (логин, диапазон дат, dry-run) и быстрый доступ ко всем CLI-командам.
Состояние сессии (email, путь к YAML, настройки LLM) сохраняется между запусками — вводить заново не нужно.

В GUI есть **Простой** и **Экспертный** режимы. Для упакованной версии Windows
нужно хранить `FitWeaver.exe` и `garmin-fit-cli.exe` вместе в одной папке.
...
| Модель | Заметки |
|--------|---------|
| `qwen/qwen3.5-9b` | ✅ Рекомендуется — лучшее качество, нативно поддерживает `enable_thinking: false` |
| `qwen/qwen3.5-4b` | ✅ Хорошо — быстрее, меньше VRAM |

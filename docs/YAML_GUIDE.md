...
**Проще всего не считать этот индекс вручную вообще** — вкладка "🧱 Конструктор" в `fitweaver_gui.py` позволяет выделить блоки в последовательности и нажать «Повторить ×N»; `back_to_offset` вычисляется автоматически из выделения.

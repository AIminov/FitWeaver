...
Начиная с v10.3 бот автоматически отключает этот режим через `extra_body` в API-запросе
и не делает лишних retry. Если проблема осталась — проверьте:

- Версия бота актуальная? (`python -m garmin_fit.bot` должна печатать v10.3+)
- В логах LM Studio НЕ должно быть `Start thinking...` после первого запроса

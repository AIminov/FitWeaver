...
- [LLM YAML Rules](LLM_YAML_RULES.md) — Правила валидации для LLM генерации
...
Primary commands now go through garmin_fit:
...
Scripts/ is now a legacy compatibility layer. Existing Scripts.* commands still work during migration, but new automation should target garmin_fit.*.


## Official Interfaces

Use these as the supported entry points:
...
- get_fit.py is still available as a compatibility entry point.
- Scripts/ remains for backward compatibility and tests.
- New code should import from garmin_fit / src.garmin_fit.

...
- системный промпт строится из `src/garmin_fit/llm/llm_contract.yaml`
- few-shot примеры берутся из `src/garmin_fit/llm/strict_examples.yaml`
...
Один запрос на весь план (было: сегментированная генерация):

- весь исходный текст уходит в модель **одним запросом** — `generate_yaml_draft()`
  не разбивает его на блоки
- разбор заголовков остался, но только как источник подсказок и фактов
  (`_extract_workout_facts_from_source_text`), а не как способ нарезать запросы
- до 2026-09-06 план из 2–10 блоков генерировался по блокам с последующей склейкой;
  от этого отказались, потому что сегментация теряла общий контекст плана и ссылки
  между днями («повтори вторничную тренировку»)
- ретраев по умолчанию два (`llm.client.MAX_RETRIES`): первичная попытка и одна
  исправляющая, в которую передаётся предыдущий ответ модели
...
Каждый запуск workflow имеет `run_id`.
...
- garmin_fit/ is a source-checkout path bridge into `src/garmin_fit`
